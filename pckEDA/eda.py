...
class AnalisisDatosExploratorio():
    ...
    def eliminarNulos(self):
        ...
        print(f"Valores nulos antes: {nulos_antes} en {self.__df.shape[0]} filas.")
        print(self.__df.isnull().sum())
        self.__df = self.__df.dropna()
        nulos_despues = self.__df.isnull().sum().sum()
        print(f"Valores nulos después: {nulos_despues} en {self.__df.shape[0]} filas.")
    ...
    def __graficosBoxplot(self):
        ...
        self.__df.boxplot(return_type='axes', ax=ax)
        plt.show()
    ...
    def graficarFrecuencias(self, title, xlabel, ylabel, columna):
        """Genera un gráfico de barras con las 40 categorías más frecuentes de una columna.
    ...
    def graficoBoxplotOriginal(self):
        """Genera un boxplot por columna numérica organizado en una cuadrícula de 3 columnas."""
